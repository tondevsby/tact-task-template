...
const { mkdirSync, cpSync, copyFileSync, rmdirSync, rmSync } = require("fs");
...
const src = {
  ...
  TACT_CONFIG_FILE: join(ROOT_DIR, "tact.config.json"),
};
...
const pack = {
  ...
  TACT_CONFIG_FILE: join(ROOT_DIR, "/pack/tact.config.json"),
};
...
exec("tsc -b", (error) => {
  if (!error) {
    ...
    copyFileSync(src.TACT_CONFIG_FILE, pack.TACT_CONFIG_FILE);
  }
});
